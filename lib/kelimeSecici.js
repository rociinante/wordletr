...
export function rastgeleKelimeUzunluklu(kategori = 'klasik', uzunluk = 5, oncekiKelime = '') {
  ...
  do {
    ...
    yeniKelime = turkceUpper(kelimeListesi[index]);
    deneme++;
  } while (yeniKelime === turkceUpper(oncekiKelime) && deneme < 10);
...
/**
 * İpucu hakkı hesapla (çok zor - sadece düşük başarı oranlarında)
 */
export function ipucuHakkiHesapla(istatistik) {
  if (!istatistik || istatistik.oynanan === 0) return 1;
  
  const basariOrani = (istatistik.kazanilan / istatistik.oynanan) * 100;
  const seri = istatistik.seri || 0;
  
  // Çok zor: sadece düşük başarı oranında ipucu
  let ipucuHakki = 0;
  ...
  // 10+ seri yapanlara 1 bonus
  if (seri >= 10) {
    ipucuHakki += 1;
  }
  
  return Math.min(ipucuHakki, 2);
}
