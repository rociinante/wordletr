...
import { temaRengiGetir, temaRengiDegistir, TEMA_RENKLERI, temaToggle, temaGetir } from '../lib/depolama';
...
export function AyarlarIcerigi() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between p-4 rounded-xl" style={{ background: 'var(--bg-tertiary)' }}>
        <div className="flex items-center gap-3">
          <span className="text-2xl">{tema === 'dark' ? '🌙' : '☀️'}</span>
          <div>
            <p className="font-semibold">Tema</p>
            <p className="text-xs opacity-60">{tema === 'dark' ? 'Karanlık' : 'Aydınlık'}</p>
          </div>
        </div>
        <button 
          onClick={handleTemaToggle}
          className={`w-14 h-8 rounded-full transition-all ${tema === 'light' ? 'bg-yellow-500' : 'bg-indigo-500'}`}
        >
          <div className={`w-6 h-6 bg-white rounded-full transition-transform ${tema === 'light' ? 'translate-x-7' : 'translate-x-1'}`} />
        </button>
      </div>
